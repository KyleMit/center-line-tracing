#!/usr/bin/env bash
# Full reproducible bench matrix for the skimage-skan track.
#   bash experiments/skimage-skan/runbench.sh
# Takes ~20 minutes.  Results merge into debug/skimage-skan/metrics.json,
# keyed by (image, tag), so re-running one section refreshes just that section.
set -euo pipefail
cd "$(dirname "$0")/../.."

B="python3 experiments/skimage-skan/bench.py"
...
python3 experiments/skimage-skan/corpus.py
...
echo "### contact sheets"
python3 experiments/skimage-skan/sheets.py comparison --tag 'medial-axis@4+pw' --crops 2
python3 experiments/skimage-skan/sheets.py comparison --tag 'medial-axis@4+pw' --corpus --crops 0 --name corpus
python3 experiments/skimage-skan/sheets.py progress --image sun-square
python3 experiments/skimage-skan/sheets.py progress --image house-wide
