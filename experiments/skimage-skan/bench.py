#!/usr/bin/env python3
"""One re-runnable bench command for the skimage-skan track.

    python3 experiments/skimage-skan/bench.py corpus
    python3 experiments/skimage-skan/bench.py inputs --images house-wide,dinosaur-wide
    python3 experiments/skimage-skan/bench.py sweep  --images house-wide --scales 1,2,4,8,12
    python3 experiments/skimage-skan/bench.py all

Writes debug/skimage-skan/metrics.json, debug/skimage-skan/graphs/*.json and
debug/skimage-skan/out/*.svg, and prints a table.
"""
...
DEBUG = REPO / "debug" / "skimage-skan"
GRAPHS = DEBUG / "graphs"
OUTDIR = DEBUG / "out"
PROMOTED = REPO / "outputs" / "skimage-skan"
...
def run_one(svg_path: Path, cfg: extract.ExtractConfig, tag: str,
    ...
    if promote:
        ...
        record["promoted"] = f"outputs/skimage-skan/{doc.id}.svg"
    return record
