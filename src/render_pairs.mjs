// Render each source drawing and its traced output to matched WebP pairs, for
// the before/after contact sheet.
//
//   node src/render_pairs.mjs [outputs/skimage-skan]
//
...
const OUT_DIR = process.argv[2] || `${REPO}outputs/skimage-skan`;
const ASSETS = `${REPO}runs/sheet-assets.json`;
const LONG_EDGE = 820;
...
mkdirSync(`${REPO}runs`, { recursive: true });
writeFileSync(ASSETS, JSON.stringify(out));
const total = out.reduce((a, r) => a + r.before.length + r.after.length, 0);
console.log(`\n${out.length} pairs -> runs/sheet-assets.json (${Math.round(total / 1024 / 1024 * 0.75)} MB of image data)`);
